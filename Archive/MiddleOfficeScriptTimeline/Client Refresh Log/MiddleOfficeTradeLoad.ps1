...
###***** Set the GenericPush variables *****
$GenericPushSQLQuery = "SELECT GenericPushJobID FROM Feeds.dbo.vGenericPushJob WHERE RefRecStatusID = 1 AND Name = '$GenericPushName'"
...
### Run the Push for the date range identified
fGenericPushTrade $GenericPushJobID -pRefDatasetDate $RefDatasetDate -pLogFile $LogFile -pScriptName $PSScriptName

### Run the proc to automatically update the ActualSettleDate in Trade Capture based on the MOS tTrade data
$ClientConfigurationSQLQuery = "SELECT [Value] FROM Core.dbo.tClientConfiguration WHERE [Module] = 'OMS' AND [Name] = 'US Bank Unsettled Trade File'"
$ClientConfigurationData = fExecuteSQL -pServerName $ServerName -pDatabaseName $Core -pQuery $ClientConfigurationSQLQuery
$ClientConfigurationValue = $ClientConfigurationData.Value

if ($ClientConfigurationValue -eq 'TRUE')	{
	$SqlQuery = 'EXEC Client.pSiepeMosSettleDateCapture'
	$SqlResult = fExecuteSQL $ServerName $Core $SqlQuery $LogFile $CommandTimeOut
}

$SqlQuery = 'EXEC Reference.Client.pSiepeMOSInstrumentDeactivation @Debug = 0'
