...
$SplitFiles = @(Get-ChildItem -Path $SourceFolder | Where-Object { $_.Name -ilike '*MOS_Aristotle Position Extract_*.csv' })
...
$SourceFolder = "\\aristotle.aws\SHARED\Clients\134\PROD\Siepe MOS\PositionCashflow\Current Day"
$ArchiveFolder = "\\aristotle.aws\SHARED\Clients\134\PROD\Siepe MOS\PositionCashflow\Current Day\Archive"
...
$SplitFiles = @(Get-ChildItem -Path $SourceFolder | Where-Object { $_.Name -ilike '*MOS_Aristotle PositionCashflow Extract_*.csv' })
if ($SplitFiles.Count -ne 1) {
	fLog -pLogFile $LogFile -pMessage "ERROR: Expected 1 PositionCashflow file after split, found $($SplitFiles.Count). Aborting."
	exit
}
...
if ($FromDate -is [datetime] -and $ToDate -is [datetime]) {
	...
	while ($RefDataSetDate -le $ToDate) {
		fGenericPush $GenericPushJobID -pRefDatasetDate $RefDatasetDate -pLogFile $LogFile -pScriptName $ScriptName

		$RefDataSetDate = $RefDataSetDate.AddDays(1)
	}
	### Run PortalCalcs for entire date range
	[string]$summaryStatus = ''
	Write-Output "################ $(Get-Date -Format 'yyyy/MM/dd HH:mm:ss:fff') :: Running PortalCalcs for $FromDate to $ToDate...  `r`n" | Out-File $LogFile -Append
	Run-PortalCalc -HostName 'http://localhost:5050' -StartDate $FromDate -EndDate $ToDate -LogFile $LogFile -PortalCalcStatus ([ref]$summaryStatus)
