...
DECLARE @CompanyID      INT       = 500000006   -- Aristotle Capital

IF @Login IS NULL
    SET @Login = SUSER_SNAME()

--Set @RefDataSetDate = '2026-06-22'        -- NULL = most recent business day

IF @RefDataSetDate IS NULL
    SET @RefDataSetDate = dbo.fDateFloorS(dbo.fOffsetDate(GETDATE(), 'W', 0), 'DD')
    select @RefDataSetDate
...
SELECT
    ...
    ,MIN(p.CreatedDate)                                             AS PositionCreatedDate
INTO #Positions
...
select count(*) as count#Positions from #Positions
...
ORDER BY
    [Asset Class], [Asset Type], [Security Name]
